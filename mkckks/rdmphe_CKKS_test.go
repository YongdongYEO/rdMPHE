...
func Test_MPHE_CKKS(t *testing.T) {
	...
	for _, expParty := range expPartyset {
		...
		Error := make([][]float64, iternum)
		for i := 0; i < iternum; i++ {
			Error[i] = make([]float64, numJoin+1)
		}
		ErrorLog := make([][]float64, iternum)
		for i := 0; i < iternum; i++ {
			ErrorLog[i] = make([]float64, numJoin+1)
		}
		...
		for iter := 0; iter < iternum; iter++ {
			...
			for _, defaultParam := range defaultParams {
				...
				ckksParams, _ := ckks.NewParametersFromLiteral(defaultParam)
				...
				*maxGroups = 1
				...
				msgVals := make([]complex128, testContext2.params.N())
				for k := 0; k < numParties; k++ {
					msgList[k], ctxtList[k] = newTestVectors(testContext2, "group0",
						complex(1.0, 0),
						complex(1.0, 0))
				}
				...
				Error[iter][logJoin], ErrorLog[iter][logJoin] = AbsMsgCKKSError(testContext2, msgVals, ctxtUpdateList[0], *testContext2.decryptor, testContext2.skSet)
		...
		Erroravg, Errorstd := meanStdFloat64(Error)
		ErrorLogavg, ErrorLogstd := meanStdFloat64(ErrorLog)
		...
		fmt.Println("Error =", Erroravg, " (std: ", Errorstd, ")")
		fmt.Println("ErrorLog =", ErrorLogavg, " (std: ", ErrorLogstd, ")")
	}
}
...
func Test_rdMPHE_CKKS(t *testing.T) {
	...
	for _, numParties := range PartySet {
		...
		// var (
		// 	KeyGen    [iternum]time.Duration
		// 	Switch    [iternum]time.Duration
		// 	EvalTime1 [iternum]time.Duration
		// 	Extend    [iternum]time.Duration
		// 	EvalTime2 [iternum]time.Duration
		// 	Error     [iternum]float64
		// 	ErrorLog  [iternum]float64
		// )
			...
			for _, defaultParam := range defaultParams {
				...
				*maxGroups = 1
				...
				var (
					...
					grlk *mkrlwe.RelinearizationKey
				...
				for j := range groupList {
					for i := 0; i < numParties2; i++ {
						msgList[i+j], ctxtList[i+j] = newTestVectors(testContext2, groupList[j], complex(1.0, 0), complex(1.0, 0))
					}
				}
				copy(msgVals, msgList[0].Value)
				...
				Error[iter][0], ErrorLog[iter][0] = AbsMsgCKKSError(testContext2, msgVals, ctxtUpdateList[0], *testContext2.decryptor, testContext2.skSet)
				...
				for JoiningParties := numParties2; JoiningParties <= (1 << numJoin); JoiningParties = JoiningParties * 2 {
					...
					fmt.Print("logJoin = ", logJoin, "\n")

					fmt.Printf("================= CKKS_rdMPHE %d ===================\n\n\n",
					...
					// // MPHE Error n parties
					// MsgCKKSError(testContext2, msgVals, ctxtUpdateList[0], *testContext2.decryptor, testContext2.skSet)
			...
			for _, defaultParam := range defaultParams {
				...
				for _, numGroups := range []int{P} {
					*maxGroups = numGroups
					...
					// msgRes := testContext2.decryptor.Decrypt(ctxtUpdateList[0], testContext2.skSet)
					// fmt.Print("ctxt after Post-Eval = ", msgRes.Value[:2], "\n")
...
func Test_rdMPHE_CKKS_fig(t *testing.T) {
...
func Test_dMPHE_CKKS_fig(t *testing.T) {
	var PartySet = [1]int{2} // , 3, 7, 15, 31
	for _, numParties := range PartySet {
		fmt.Printf("Number of Parties Before Join = %d\n", numParties)
...
func Test_MKHE_CKKS_fig(t *testing.T) {
	var PartySet = [1]int{2} // , 3, 7, 15, 31
	for _, P := range PartySet {
		fmt.Printf("P = %d\n", P)
...
func VectorProd_Before_Join(testContext *testParams, userList []string, numParties int, sk []*mkrlwe.SecretKey, swk []*mkrlwe.SWK, swkhead []*mkrlwe.SWK, flag int, t *testing.T) (ctxtout *Ciphertext, Switchtemp time.Duration, MultBtemp time.Duration) {
	...
	for j := range userList {
		for i := 0; i < numParties; i++ {
			msgList[i+j], ctList[i+j] = newTestVectors(testContext, userList[j], complex(1.0, 0), complex(1.0, 0))
		}
	}
...
func VectorProd_After_Join(testContext *testParams, userList []string, numParties int, JoiningParties int, ctxtin *Ciphertext, flag int, t *testing.T) (ctxtout *Ciphertext, MultAtemp time.Duration) {
	...
	if flag == 0 {
		for i := 0; i < JoiningParties; i++ {
			msgList[i], ctList[i] = newTestVectors(testContext, "group0", complex(1.0, 0), complex(1.0, 0))
		}
	} else {
		for i := 0; i < JoiningParties; i++ {
			msgList[i], ctList[i] = newTestVectors(testContext, "group"+strconv.Itoa(len(userList)+i-1), complex(1.0, 0), complex(1.0, 0))
		}
	}
...
func testEvaluatorSub(testContext *testParams, userList []string, t *testing.T) {
	...
	t.Run(GetTestName(testContext.params, "MKCKKSSub: "+strconv.Itoa(numUsers)+"/ "), func(t *testing.T) {
		ctRes := ct
...
func testEvaluatorMul(flag int, testContext *testParams, userList []string, t *testing.T) {
	...
	// // Test
	// msgRes1 := testContext.decryptor.Decrypt(ct, testContext.skSet)
	// fmt.Print("Multiplicand 1 = ", msgRes1.Value[:10], "\n")
	// msgRes2 := testContext.decryptor.Decrypt(ct2, testContext.skSet)
	// fmt.Print("Multiplicand 2 = ", msgRes2.Value[:10], "\n")
...
func Join_dMPHE(testContext *testParams,
	...
	grlk *mkrlwe.RelinearizationKey, gcjk *mkrlwe.ConjugationKey, grtk *mkrlwe.RotationKey,
	...
	grlkup *mkrlwe.RelinearizationKey, gcjkup *mkrlwe.ConjugationKey, grtkup *mkrlwe.RotationKey,
	...
	for p := 0; p < JoiningParties; p++ {
		...
		for i := 0; i < beta; i++ {
			ringQP.AddLvl(levelQ, levelP, grlk.Value[0].Value[i], rlkNew.Value[0].Value[i], grlk.Value[0].Value[i])
			ringQP.AddLvl(levelQ, levelP, grlk.Value[1].Value[i], rlkNew.Value[1].Value[i], grlk.Value[1].Value[i])
			ringQP.AddLvl(levelQ, levelP, grlk.Value[2].Value[i], rlkNew.Value[2].Value[i], grlk.Value[2].Value[i])
			ringQP.AddLvl(levelQ, levelP, gcjk.Value.Value[i], cjkNew.Value.Value[i], gcjk.Value.Value[i])
		}
...
func Join_rdMPHE(testContext *testParams,
	...
	gskup *mkrlwe.SecretKey, gpkup *mkrlwe.PublicKey,
	grlkup *mkrlwe.RelinearizationKey, gcjkup *mkrlwe.ConjugationKey, grtkup *mkrlwe.RotationKey, skup []*mkrlwe.SecretKey,
	jkup *mkrlwe.SWK, jkheadup *mkrlwe.SWK,
	uauxup *mkrlwe.SWK, uauxheadup *mkrlwe.SWK,
	ctxtListup []*Ciphertext, ExtGentemp, ExtCttemp time.Duration) {
...
func MsgCKKSError(
	...
	dec Decryptor,
	skSet *mkrlwe.SecretKeySet,
) (totalAbsErr float64, totalLogErr float64) {
	params := testContext.params
	slots := params.Slots()

	// var totalAbsErr float64
	// var totalLogErr float64
	var count float64

	msgOut := dec.Decrypt(ct, testContext.skSet)

	for i := 0; i < slots; i++ {

		errVal := real(msgVals[i] - msgOut.Value[i])
		...
		totalAbsErr += absErr
		totalLogErr += math.Log2(absErr)
		count++
	}

	if count == 0 {
		fmt.Println("No non-zero errors found.")
		return
	}

	avgAbsError := totalAbsErr / count
	avgLogError := totalLogErr / count

	fmt.Printf("============================================\n")
	fmt.Printf(" CKKS Error Statistics\n")
	fmt.Printf("--------------------------------------------\n")
	fmt.Printf(" Avg absolute error      : %.6e\n", avgAbsError)
	fmt.Printf(" Avg log2(error)         : %.3f bits\n", avgLogError)
	fmt.Printf("============================================\n")

	return avgAbsError, avgLogError
}

func AbsMsgCKKSError(
	testContext *testParams,
	msgVals []complex128,
	ct *Ciphertext,
	dec Decryptor,
	skSet *mkrlwe.SecretKeySet,
) (infAbsErr float64, logInfAbsErr float64) {
	...
	if maxAbsErr == 0 {
		fmt.Println("No error detected (all zero).")
		return 0, math.Inf(-1)
	}

	logErr := math.Log2(maxAbsErr)

	fmt.Printf("============================================\n")
	fmt.Printf(" CKKS Error (Infinity Norm)\n")
	fmt.Printf("--------------------------------------------\n")
	fmt.Printf(" ||e||_∞                : %.6e\n", maxAbsErr)
	fmt.Printf(" log2(||e||_∞)          : %.3f bits\n", logErr)
	fmt.Printf("============================================\n")

	return maxAbsErr, logErr
}
...
func PtxtCKKSError(
	...
	// --------------------------------------------
	// 1) Ideal plaintext (Encode only)
	// --------------------------------------------
	ptIdeal := dec.encoder.EncodeNew(
		msgVals,
		params.LogSlots(),
	)

	// 2) Decrypt only (NO Decode)
	ptDec := rlwe.NewPlaintext(params.Parameters.Parameters, ct.Level())
	testContext.decryptor.Decryptor.Decrypt(ct.Ciphertext, testContext.skSet, ptDec)
	// ptDec.Scale = ct.Scale
	...
	diff := ringQ.NewPoly()
	ringQ.Sub(ptDec.Value, ptIdeal.Value, diff)

	// 4) L2-norm computation
	...
	fmt.Printf("============================================\n")
	fmt.Printf(" CKKS Plaintext Error (No Decode)\n")
	fmt.Printf("        L2-norm in coefficient domain\n")
	fmt.Printf("--------------------------------------------\n")
	fmt.Printf(" ||e||_2           : %.6e\n", l2norm)
	fmt.Printf(" log2(||e||_2)     : %.3f bits\n", math.Log2(l2norm))
	fmt.Printf("============================================\n")
}
