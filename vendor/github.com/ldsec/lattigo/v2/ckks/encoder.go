...
func newEncoder(params Parameters) encoder {
	...
	gaussianSampler := ring.NewGaussianSampler(prng, params.RingQ(), params.Sigma(), uint64(6*params.Sigma()))
...
func (encoder *encoderComplex128) decodePublic(plaintext *Plaintext, logSlots int, sigma float64) (res []complex128) {
	...
	if sigma != 0 {
		encoder.gaussianSampler.ReadAndAddFromDistLvl(plaintext.Level(), encoder.polypool, encoder.params.RingQ(), sigma, uint64(2.5066282746310002*sigma))
	}
...
func (encoder *encoderBigComplex) decodePublic(plaintext *Plaintext, logSlots int, sigma float64) (res []*ring.Complex) {
	...
	if sigma != 0 {
		// B = floor(sigma * sqrt(2*pi))
		encoder.gaussianSampler.ReadAndAddFromDistLvl(plaintext.Level(), encoder.polypool, encoder.params.RingQ(), sigma, uint64(2.5066282746310002*sigma+0.5))
	}
