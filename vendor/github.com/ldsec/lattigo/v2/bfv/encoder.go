...
func (encoder *encoder) DecodeInt(p interface{}, coeffs []int64) {
	...
	// Q := big.NewInt(1)
	// for _, qi := range encoder.params.RingQ().Modulus {
	// 	Q.Mul(Q, new(big.Int).SetUint64(qi))
	// }
	// t := big.NewInt(int64(encoder.params.T()))
	// denom := new(big.Int).Mul(t, big.NewInt(2))

	// Qover2t := new(big.Int).Div(Q, denom)
	// log2Qover2t := float64(Qover2t.BitLen() - 1)

	// fmt.Printf("Q / (2t) ≈ 2^%.2f\n", log2Qover2t)
}
